...
const UserSchema = new mongoose.Schema({
    ...
    password: String,
    bio: { type: String, default: "Aero Music Fan" },
    friends: [String]
});
...
// 3. MIDDLEWARE
app.use(session({ secret: 'aero-secret-2026', resave: false, saveUninitialized: false }));
...
app.post('/login', async (req, res) => {
    ...
    } else { res.send("Invalid Login. <a href='/login.html'>Back</a>"); }
});

app.get('/api/posts', protect, async (req, res) => res.json(await Post.find().sort({ date: -1 })));

app.post('/api/post', protect, async (req, res) => {
    await new Post({ author: req.session.nickname, content: req.body.content }).save();
...
app.post('/api/delete-comment', protect, async (req, res) => {
    await Post.findByIdAndUpdate(req.body.postId, {
        $pull: { comments: { _id: req.body.commentId, author: req.session.nickname } }
    });
    res.redirect('/social.html');
});
...
app.get('/api/profile/:name', protect, async (req, res) => {
    ...
    res.json({ user, posts, isMe: req.session.nickname === req.params.name, myName: req.session.nickname });
});

app.post('/api/update-profile', protect, async (req, res) => {
    await User.findOneAndUpdate({ nickname: req.session.nickname }, { bio: req.body.bio });
